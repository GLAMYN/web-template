...
module.exports = (req, res) => {
  ...
  const isAcceptanceFromCardSaved = bodyParams?.transition === 'transition/accept-from-card-saved';
  const transactionId = bodyParams?.id?.uuid || bodyParams?.id;
  const transactionPromise = isAcceptanceFromCardSaved && transactionId
  ...
  Promise.all([listingPromise(), fetchCommission(sdk), currentUserPromise, transactionPromise])
    .then(
      async ([
        showListingResponse,
        fetchAssetsResponse,
        ...
        const bookingEndFromTx = transaction?.booking?.attributes?.end;
        const commissionAsset = fetchAssetsResponse.data.data[0];

        // Important: for acceptance, the currentUser is the PROVIDER.
        // We need the CUSTOMER's ID for commission and previous transaction history.
        const customerId = isAcceptanceFromCardSaved
          ? transaction.relationships.customer.data.id.uuid
          : currentUserResponse.data.data.id.uuid;

        stripeCustomerId =
          currentUserResponse.data.data.relationships?.stripeCustomer?.data?.id?.uuid;

        // Ensure we have a listingId even if not passed in params
        const listing_Id =
          bodyParams?.params?.listingId?.uuid ||
        ...
        const balanceAdj = lineItems.find(
          (item) => item.code === 'line-item/pip-balance-adjustment'
        );
        pipProtectedData = {};
      ...
      if (isFarFuture && !isSpeculative) {
        try {
          if (stripeCustomerId) {
            ...
            console.error(
              'transition-privileged - stripeCustomerId missing, cannot create SetupIntent'
            );
          }
        } catch (error) {
